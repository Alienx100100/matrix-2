...
def clean_expired_users():
    try:
        ...
        for user in removed_users:
            ...
            # Send enhanced message to removed user
            user_message = f"""
🚫 𝗦𝘂𝗯𝘀𝗰𝗿𝗶𝗽𝘁𝗶𝗼𝗻 𝗘𝘅𝗽𝗶𝗿𝗲𝗱

👤 𝗨𝘀𝗲𝗿: @{username}
🔑 𝗞𝗲𝘆: {key}
⏰ 𝗘𝘅𝗽𝗶𝗿𝗲𝗱 𝗮𝘁: {expiration.strftime('%Y-%m-%d %H:%M:%S')} IST
...
"""
        ...
        if removed_users:
            ...
            for user in removed_users:
                admin_message += f"👤 @{user[1]} (ID: {user[0]})\n"
...
"""
    ...
    try:
        # Execute attack
        subprocess.run(f"./matrix {target} {port} {time}", shell=True)
